...
bool clip_granite4_newline_row(struct clip_ctx * ctx, std::vector<float> & out) {
    ...
    GGML_ASSERT(model.image_newline->type == GGML_TYPE_F32);
    const int proj_dim = model.hparams.projection_dim;
    const int K        = (int) model.qf_proj_blocks.size();
    GGML_ASSERT(proj_dim > 0 && K > 0);
    GGML_ASSERT((int) ggml_nelements(model.image_newline) == proj_dim);

    // the stored newline is a single projection_dim vector; the full per-token
    // embedding tiles it once per deepstack projector block (see build_newline_row)
    std::vector<float> nl(proj_dim);
    ggml_backend_tensor_get(model.image_newline, nl.data(), 0, ggml_nbytes(model.image_newline));
    out.resize((size_t) K * proj_dim);
    for (int k = 0; k < K; ++k) {
        std::copy(nl.begin(), nl.end(), out.begin() + (size_t) k * proj_dim);
    }
    return true;
}
